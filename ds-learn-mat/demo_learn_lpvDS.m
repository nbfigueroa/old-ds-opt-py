...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%  Step 1 - DATA LOADING: Load drawn data from Python-GUI %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear all; close all; clc;

% Define package path and dataset name
pkg_dir      = '/home/nbfigueroa/Dropbox/MIT-IRG/PostDoc-papers/Constrained-DS/ds-opt-python/';
dataset_name = strcat(pkg_dir,'/data/human_demonstrated_trajectories.mat');
...
if adjusts_C  == 1 
    if M == 2
        tot_dilation_factor = 1.2; rel_dilation_fact = 0.25;
    elseif M == 3
...
if constr_type == 0 || constr_type == 1
    ...
    %     [Vxf] = learn_wsaqf(Data,0,att);
        ...
        % Testing this idea
        % Fit Single Gaussian to Trajectory Data
        est_options_P = [];
        est_options_P.type     = 1;   % GMM Estimation Alorithm Type   
        est_options_P.maxK     = 1;   % Fix K and estimate with EM for Type 1
        est_options_P.fixed_K  = 1;   % Fix K and estimate with EM for Type 1
        est_options_P.do_plots = 0;
        est_options_P.sub_sample = 1;   % Size of sub-sampling of trajectories    
        [~, ~, Sigma] = fit_gmm(Xi_ref, [], est_options_P);
        
        % Construct P from overall covariance of data
        [V,L] = eig(Sigma);
        V_P   = [V(:,2) V(:,1)];
        L_P   = 100*diag([L(2,2),L(1,1)]);
        P_opt = V_P*(L_P)*V_P';
        P_opt = Sigma;
    end
end
